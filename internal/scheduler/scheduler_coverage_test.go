...
func TestRunPluginSyncTicksRepeatedly(t *testing.T) {
	...
	if err := pluginStore.UpdateManifest(ctx, "ticker-plugin", true, sql.NullString{}); err != nil {
...
func TestSyncAllPluginsLogsErrorWhenSyncOneFails(t *testing.T) {
	...
	if err := pluginStore.UpdateManifest(ctx, "broken-plugin", true, sql.NullString{}); err != nil {
		t.Fatalf("UpdateManifest: %v", err)
	}
