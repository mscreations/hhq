...
| `hhq.extraMounts[].secretName`          | (unset)              | One key per file referenced by a `parents[].password_file` or `calendars[].password_file` (see below) |
| `bootstrap.external`                    | (unset)              | A raw volume source (Secret, ConfigMap, PVC, etc) containing the bootstrap JSON files, if using `bootstrap.external` instead of the inline lists (see below) |
...
- `bootstrap.parents` / `bootstrap.children` / `bootstrap.chores` /
  ...
  mounted via `hhq.extraMounts`, see below) rather than a plain
...
- `hhq.extraMounts` - mounts one or more existing Secrets (created outside
  this chart, never templated here) into the hhq container, for the
  ...
  `extraMounts` (see below). Applies regardless of which bootstrap mode is
  in use.
- `plugins.json` is generated automatically from `.Values.plugins` - you
  never write it by hand, and it's always mounted at `CONFIG_DIR/plugins.json`
  on top of whichever bootstrap mode you're using below.
...
### Alternative: an externally-managed bootstrap directory

Instead of the inline lists above, `bootstrap.external` mounts an
already-populated directory (any raw Kubernetes volume source - a Secret,
ConfigMap, PVC, hostPath, etc, managed entirely outside this chart) directly
at `bootstrap.configDir`, containing whichever of `parents.json`/
`children.json`/`chores.json`/`assignments.json`/`calendars.json` you need.
This is **mutually exclusive** with `bootstrap.parents`/`children`/`chores`/
`assignments`/`calendars` above - `helm template`/`helm install` fails if
both are set. `plugins.json` is still auto-generated by this chart and
mounted on top, regardless of which mode you use.
...
```sh
kubectl create secret generic hhq-bootstrap-files \
  --from-file=parents.json --from-file=calendars.json -n hhq
```
